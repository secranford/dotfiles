...
#open relevant us3d doc files, change location to match
uman() {
    local default="$LOCAL/codes/us3d-files/version/1.2.0/doc/US3D-User-Manual.pdf" #change to what you want default file to be
    local file

    if [[ "$1" == "1.1.7" || "$1" == "1.1.8" ]]; then
        # for 1.1.7 or 1.1.8, open the “US3D-Manual.pdf”
        file="$LOCAL/codes/us3d-files/version/$1/doc/US3D-Manual.pdf"
    elif [ $# -gt 0 ]; then
        # File name for later versions is US3D-User-Manual
        file="$LOCAL/codes/us3d-files/version/$1/doc/US3D-User-Manual.pdf"
    else
        # no arg, fallback to default
        file="$default"
    fi
...
uqik() {
    local default="$LOCAL/codes/us3d-files/version/1.2.0/doc/US3D-Quickstart-Guide.pdf"
    local file

    if [ $# -gt 0 ]; then
      file="$LOCAL/codes/us3d-files/version/$1/doc/US3D-Quickstart-Guide.pdf"
    else
      file="$default"
    fi
...
udevm() {
    ...
    if [[ "$1" == "1.1.7" || "$1" == "1.1.8" ]]; then
      ...
      file="$LOCAL/codes/us3d-files/version/$1/doc/US3D-Developer-Manual.pdf"
    else
      file="$default"
    fi
